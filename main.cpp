...
static uint8_t global_server_mac[6];
static uint16_t global_connection = 0;
static uint16_t global_dport;
...
static u_int32_t print_pkt (struct nfq_data *tb)
{
    ...
    if(flag == 0x10 && global_connection == 0)
    {
        ...
        global_Seq_number = ntohl(data_tcp_header->ack);
        global_Ack_number = ntohl(data_tcp_header->seq);
        memcpy(global_client_ip, data_ip_header->s_ip, 4);
        global_dport = ntohs(data_tcp_header->s_port);
    ...
    else if(ntohs(data_tcp_header->d_port) == 0x0050 && ntohs(data_tcp_header->win_size) == 0x1212 && flag == 0x18 && global_connection == 1)
    {
        // ...
...
int main(int argc, char **argv)
{
    ...
    char * dev = "eth1";
    GET_MY_IP_MAC(dev, global_server_ip, global_server_mac);
